...
`scripts/compile-layout.mjs` generates an authored radial transformation using the Ringbahn's angular radius profile around an explicit origin. All modes pass through the same transformation. Paths and stops maintain exactly matching indexes/identities; the layout records the exact runtime-network hash. Geographic distance and metric scale change. The diagram is not an operator map or measured physical geometry.

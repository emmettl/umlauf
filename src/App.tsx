...
function Study({data,layout}:{data:NetworkSnapshot;layout:SpatialLayoutSnapshot}) {
  ...
  const [rate,setRate]=useState(30),[crossings,setCrossings]=useState(false),[mix,setMix]=useState(0)
  ...
  const network=useMemo(()=>subset(data,crossings),[data,crossings])
  ...
  return <main className="study">
    <aside className="sidebar">
      ...
      <div className="composition" aria-label="Composition">
        <button aria-pressed={!crossings} onClick={()=>{setCrossings(false);reset()}}><span>01</span> The ring <i>↻ ↺</i></button>
        <button aria-pressed={crossings} onClick={()=>{setCrossings(true);reset()}}><span>02</span> Ring & crossings <i>↗</i></button>
      </div>

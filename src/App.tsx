...
function subset(data:NetworkSnapshot, crossings:boolean, family:Family):NetworkSnapshot {
  const trains=data.trains.filter(t=>isRing(t) || (crossings && (family==='all' || (FAMILIES[family] as readonly string[]).includes(t.route))))
...
function Study({data,layout}:{data:NetworkSnapshot;layout:SpatialLayoutSnapshot}) {
  ...
  const network=useMemo(()=>subset(data,crossings||section,family),[data,crossings,section,family])

...
function Study({data,layout}:{data:NetworkSnapshot;layout:SpatialLayoutSnapshot}) {
  ...
  const framing=useMemo(()=>({...desktopFraming,homeDistanceScale:phone?1.18:0.96,stationLabelHeightScale:phone?0.45:0.8}),[phone])
  ...
  const network=useMemo(()=>subset(data,crossings||section,family,direction),[data,crossings,section,family,direction])
  ...
  return <main className="study">
    <aside className="sidebar">
      ...
      <div className="direction-grid" aria-label="Active scheduled Ringbahn journeys">
        <button className="direction-card" aria-label="S41 clockwise only" aria-pressed={direction==='S41'} disabled={section} onClick={()=>inspectDirection(direction==='S41'?'both':'S41')}><span className="route amber">S41 <b>↻</b></span><strong data-testid="s41-count">{ringCounts[0]}</strong><small>clockwise</small></button>
        <button className="direction-card" aria-label="S42 counter-clockwise only" aria-pressed={direction==='S42'} disabled={section} onClick={()=>inspectDirection(direction==='S42'?'both':'S42')}><span className="route mint">S42 <b>↺</b></span><strong data-testid="s42-count">{ringCounts[1]}</strong><small>counter-clockwise</small></button>
        <button className="both-directions" aria-pressed={direction==='both'} disabled={section} onClick={()=>inspectDirection('both')}>Both directions</button>
      </div>
    ...
    <section className="field" aria-label="Animated Berlin railway study">
      ...
      {section ? <Suspense fallback={<p className="loading">Preparing Ostkreuz…</p>}><OstkreuzScene network={data} time={time} playing={playing} rate={rate} onTime={setTime} onInspectCall={next=>{setTime(next);setPlaying(false)}}/></Suspense> : <><div className="canvas" data-testid="network-scene"><NationalNetworkScene lakes={waterEnabled&&mix<1?water:undefined} snapshot={network} referenceSnapshot={reference} isPlaying={playing} time={time} onTime={setTime} playbackRate={rate} stations={stations} trainLabelMode={labels?'on':'off'} cameraFraming={framing} cameraCommand={camera} selectedStation={station} selectedTrain={train} onSelectStation={s=>{setStation(s);setTrain(undefined)}} spatialLayout={layout} spatialLayoutMix={mix} routeColors={COLORS} routeColorMix={1} stationLabelTierLimit={phone&&!labels?0:2} topologicalStyle="line-map" groundStyle="quiet"/></div>
      ...
      <div className="map-tools"><button aria-label="Zoom in" onClick={()=>setCamera({id:++cameraId.current,action:'zoom-in'})}>+</button><button aria-label="Zoom out" onClick={()=>setCamera({id:++cameraId.current,action:'zoom-out'})}>−</button><button aria-label="Reset view" onClick={reset}>↺</button><button aria-label="Map labels" aria-pressed={labels} onClick={()=>setLabels(!labels)}>Aa</button></div>

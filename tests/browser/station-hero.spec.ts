...
test('a map station opens a responsive departure card and selects a train', async ({ page }) => {
 ...
 await expect(card.locator('.ms-vehicle-hero')).toBeVisible()
 await expect(card.locator('.ms-vehicle-hero__next')).toBeVisible()
 expect(await card.evaluate(element=>element.scrollWidth<=element.clientWidth+1)).toBe(true)
 await card.screenshot({path:'test-results/vehicle-hero.png'})
 await page.getByRole('button',{name:'Back to departures'}).click()
 await expect(card.locator('.ms-dot-matrix-board')).toBeVisible()
